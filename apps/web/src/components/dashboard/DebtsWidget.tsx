    ...
    return (
        <Card className={cn('dashboard-widget h-full flex flex-col group rounded-lg overflow-hidden', isCompact && 'dashboard-widget--compact')}>
                <CardHeader className="p-3 pb-1 flex flex-row items-start justify-between hover:bg-muted/30 transition-colors rounded-t-lg cursor-pointer">
                    <div className="flex flex-col min-w-0 flex-1">
                        ...
                        <div className="flex items-baseline gap-2 flex-wrap">
                            <span className="text-lg font-bold tracking-tight whitespace-nowrap text-rose-600 dark:text-rose-500">
                                <CurrencyDisplay amount={-totalIOwe} abbreviate={totalIOwe >= 1000} />
                            </span>
                            {totalTheyOwe > 0 && (
                    ...
                    <div className="p-1.5 bg-amber-500/10 rounded-sm group-hover:bg-amber-500/20 transition-colors">
                        ...
                        {activeDebts.length === 0 ? (
                            ...
                            <>
                                {visibleIOwe.map((debt: any) => (
                                    <div key={debt.id} className="flex items-center justify-between gap-2 p-1.5 rounded-sm bg-rose-500/5 border border-rose-500/10 hover:bg-rose-500/10 transition-colors group/item">
                                ...
                                {visibleTheyOwe.map((debt: any) => (
                                    <div key={debt.id} className="flex items-center justify-between gap-2 p-1.5 rounded-sm bg-emerald-500/5 border border-emerald-500/10 hover:bg-emerald-500/10 transition-colors group/item">
                                        <div className="flex items-center gap-1.5 min-w-0 flex-1">
