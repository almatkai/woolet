...
import { TrendingDown, TrendingUp, ArrowDownRight, CreditCard, Calendar } from 'lucide-react';
...
export function MonthlyExpensesWidget({ gridParams }: { gridParams?: GridParams }) {
    ...
    const { data: expenseData, isLoading: isCurrentLoading } = trpc.transaction.list.useQuery({
        type: 'expense',
    ...
    const { data: prevExpenseData, isLoading: isPrevLoading } = trpc.transaction.list.useQuery({
        type: 'expense',
    ...
    const prevTransactions = prevExpenseData?.transactions || [];
    const prevTotalExpense = prevTransactions.reduce((sum: number, t: Transaction) => sum + Number(t.amount), 0);
    
    const diff = totalExpense - prevTotalExpense;
    // For expenses, if diff > 0 (spent more), it's trending UP in spending (bad)
    const percentChange = prevTotalExpense > 0 ? (diff / prevTotalExpense) * 100 : 0;
    ...
    return (
        <Card className={cn('dashboard-widget h-full flex flex-col group overflow-hidden', isCompact && 'dashboard-widget--compact')}>
            <div className="flex-1 flex flex-col min-h-0 min-w-0">
                <CardHeader className="p-3 pb-1 flex flex-row items-start justify-between">
                    <div className="flex flex-col min-w-0 flex-1">
                        <div className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Monthly Expenses</div>
                        <div className="flex items-baseline gap-1.5 flex-wrap">
                            <span className="text-lg sm:text-xl font-bold tracking-tight text-rose-600 dark:text-rose-500 whitespace-nowrap">
                                <CurrencyDisplay amount={-totalExpense} abbreviate={totalExpense > 1000000} />
                            </span>
                            <div className={cn(
                                ...
                                {diff <= 0 ? <TrendingDown className="h-2.5 w-2.5 mr-0.5" /> : <TrendingUp className="h-2.5 w-2.5 mr-0.5" />}
                ...
                <CardContent className="px-3 py-1 flex-1 flex flex-col min-h-0 relative">
                    ...
                    <div className="relative z-10 py-1">
                        <div className="flex items-center justify-between text-[10px] mb-1">
                            <span className="text-muted-foreground">Prev: <CurrencyDisplay amount={-prevTotalExpense} abbreviate /></span>
                            <span className={cn("font-bold", diff <= 0 ? "text-emerald-600" : "text-rose-600")}>
                                {diff <= 0 ? '-' : '+'}<CurrencyDisplay amount={Math.abs(diff)} abbreviate />
                            </span>
                        </div>
                        <div className="w-full h-1 bg-muted rounded-full overflow-hidden">
                            <div 
                                className="h-full bg-rose-500 rounded-full" 
                                style={{ width: `${Math.min(100, (totalExpense / (prevTotalExpense || totalExpense || 1)) * 100)}%` }} 
                            />
                        </div>
                    </div>
                    ...
                    {isTall && transactions.length > 0 && (
                        <div className="mt-2 space-y-1 overflow-hidden relative z-10">
                            {transactions.slice(0, 2).map(tx => (
                                <div key={tx.id} className="flex items-center justify-between p-1.5 rounded-md bg-background/60 backdrop-blur-sm border border-rose-500/10">
                                    <div className="flex items-center gap-1.5 min-w-0">
                                        <span className="text-sm">{tx.category?.icon || '💸'}</span>
                                        <span className="text-[10px] font-bold truncate leading-tight">{tx.description || tx.category?.name || 'Expense'}</span>
                                    </div>
            ...
            <WidgetFooter>
                ...
                <Link to="/spending" className="dashboard-widget__footer-action text-[9px] font-bold text-rose-600 hover:underline uppercase tracking-wider">
                    Details
                </Link>
            </WidgetFooter>
        </Card>
