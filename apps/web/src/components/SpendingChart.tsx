...
import { Save } from 'lucide-react'; // Added Save and Trash icons
            ...
            <CardHeader className="pb-2 px-3 pt-3">
                <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                        <CardTitle className="dashboard-widget__title text-xs sm:text-sm truncate font-bold">Spending Overview</CardTitle>
                        ...
                        >
                            ...
                            <div className="absolute inset-0 p-4 pt-6 pb-8 flex">
                                ...
                                <div className="w-10 flex flex-col justify-between text-right pr-2 pb-6">
                                    {(() => {
                                        const maxAmount = Math.max(...(stats?.timeSeriesData || []).map((d: any) => d.amount), 1);
                                ...
                                <div className="flex-1 flex flex-col relative">
                                    ...
                                    {/* Bars container */}
                                    <div className="flex-1 flex items-end justify-around gap-1 relative z-10">
                                        {(stats?.timeSeriesData || []).map((item: any, index: number) => {
                                            const maxAmount = Math.max(...(stats?.timeSeriesData || []).map((d: any) => d.amount), 1);
                                            const heightPercent = (item.amount / maxAmount) * 100;
                                            return (
                                                <div key={index} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                                                    {/* Bar - transparent with backdrop blur so grid lines show through */}
                                                    <div 
                                                        className="w-full max-w-[40px] rounded-t-lg border-2 border-purple-500 backdrop-blur-[3px] transition-all duration-300 hover:scale-105 cursor-pointer"
                                                        style={{ 
                                                        ...
                                                        onMouseEnter={(e) => {
                                                            ...
                                                            if (tooltip && dateEl && amountEl) {
                                                                dateEl.textContent = format(new Date(item.date), 'EEE, MMM do');
                                                    ...
                                                    />
                                                    {/* X-axis label */}
                                                    <span className="text-[10px] text-muted-foreground/60 font-medium h-5 flex items-center">
                                                        {format(new Date(item.date), period === 'weekly' ? 'EEE' : 'dd')}
                                                    </span>
                                                </div>
                                            );
                                        })}
