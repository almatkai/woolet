...
export const splitBillRouter = router({
    ...
    // Get all pending splits (who owes you money)
    getPendingSplits: protectedProcedure
        .input(getPendingSplitsQuerySchema.optional())
        .query(async ({ ctx, input }) => {
            ...
            const allowedBalanceIds = userBanks.flatMap(b =>
                b.accounts.flatMap(a => a.currencyBalances.map(cb => cb.id))
            );
    ...
    getOwedSummary: protectedProcedure
        .query(async ({ ctx }) => {
            // Get all pending splits grouped by participant
            const userBanks = await ctx.db.query.banks.findMany({
                where: and(
                    eq(banks.userId, ctx.userId!),
                    eq(banks.isTest, ctx.user.testMode)
            ...
            const byParticipant: Record<string, { participant: any; totalOwed: number; totalPaid: number }> = {};

            for (const tx of transactionsWithSplits) {
                for (const split of tx.splits) {
                    if (!byParticipant[split.participantId]) {
                        byParticipant[split.participantId] = {
                            participant: split.participant,
                            totalOwed: 0,
                            totalPaid: 0,
                        };
                    }
                    byParticipant[split.participantId].totalOwed += Number(split.owedAmount);
                    byParticipant[split.participantId].totalPaid += Number(split.paidAmount);
                }
            }

            const summary = Object.values(byParticipant).map(p => ({
                ...p,
                remaining: p.totalOwed - p.totalPaid,
            })).filter(p => p.remaining > 0);

            const total = summary.reduce((sum, p) => sum + p.remaining, 0);

            return {
                total,
                byParticipant: summary.sort((a, b) => b.remaining - a.remaining),
            };
        }),
});
