...
import { ArrowLeft, CreditCard, Calendar, Percent, Landmark, Plus } from 'lucide-react-native';
...
const styles = StyleSheet.create({
    ...
    badgeTextActive: { color: colors.blue },
...
export default function CreditsScreen() {
    ...
    const stats = React.useMemo(() => {
        if (!credits) return { totalRemaining: 0, totalMonthly: 0 };
        return credits.reduce((acc, credit: any) => {
            acc.totalRemaining += Number(credit.remainingBalance);
            acc.totalMonthly += Number(credit.monthlyPayment);
            return acc;
        ...
        <ScreenWrapper style={styles.container}>
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.text} />}>
                <View style={styles.header}>
                    ...
                    <View><Text style={styles.headerTitle}>Credits</Text><Text style={styles.headerSubtitle}>Credit lines and loans</Text></View>
                </View>
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        {(!credits || credits.length === 0) ? (
                            <View style={styles.emptyCard}><CreditCard size={48} color={colors.textSecondary} /><Text style={styles.emptyText}>No credits or active loans</Text></View>
                        ) : (
                            credits.map((credit: any) => {
                                const payoffPercent = Math.round(((Number(credit.principalAmount) - Number(credit.remainingBalance)) / Number(credit.principalAmount)) * 100);
                                return (
                                    <View key={credit.id} style={styles.card}>
                                        <View style={styles.cardHeader}>
                                            <View style={{ flex: 1 }}><Text style={styles.cardTitle}>{credit.name}</Text><Text style={styles.cardMeta}>{credit.account?.name || 'Account'}</Text></View>
                                            <View style={[styles.badge, credit.status === 'active' ? styles.badgeActive : styles.badgePaid]}><Text style={[styles.badgeText, credit.status === 'active' ? styles.badgeTextActive : styles.badgeTextPaid]}>{credit.status === 'paid_off' ? 'Paid Off' : credit.status}</Text></View>
                                        </View>
                                        <View style={styles.progressBar}><View style={[styles.progressFill, { width: `${payoffPercent}%` }]} /></View>
                                        <View style={styles.statsRow}>
                                            <View style={styles.statItem}><Text style={styles.statLabel}>Rate</Text><Text style={styles.statValue}>{credit.interestRate}%</Text></View>
                                            <View style={styles.statItem}><Text style={styles.statLabel}>Monthly</Text><Text style={styles.statValue}>{formatCurrency(Number(credit.monthlyPayment))}</Text></View>
                                            <View style={styles.statItem}><Text style={styles.statLabel}>Remaining</Text><Text style={styles.statValue}>{formatCurrency(Number(credit.remainingBalance))}</Text></View>
                                        </View>
                                    </View>
                                );
                            })
